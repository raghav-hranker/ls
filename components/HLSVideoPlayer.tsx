...
import { PlayCircle, PauseCircle, RotateCcw, RotateCw, Volume2, VolumeX, Settings, Check, Loader2 } from "lucide-react"
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
const HLSVideoPlayer: React.FC<HLSVideoPlayerProps> = ({ srcUrl, status, roomId, streamStatus, fileGenerated, hlsConfig }) => {
    ...
    const [quality, setQuality] = useState<{ label: string; level: number }>({ label: "auto", level: -1 })
    const [isBuffering, setIsBuffering] = useState(false)
    const [bufferingTimer, setBufferingTimer] = useState<NodeJS.Timeout | null>(null)
    ...
    const handleQualityChange = (qualityLabel: string) => {
        ...
        if (qualityLabel === "auto") {
            ...
            const selectedLevel = levels?.findIndex(
                (level) => level?.height === Number.parseInt(qualityLabel.replace("p", "")),
    ...
    useEffect(() => {
        ...
        if (player) {
            const handleLevelSwitching = (event: typeof Hls.Events.LEVEL_SWITCHING, data: LevelSwitchingData) => {
                const newLevel = data.level
                const newQuality =
                    newLevel === -1
                        ? { label: "auto", level: -1 }
                        : { label: `${player?.levels[newLevel]?.height}p`, level: newLevel }
    ...
    if (streamStatus === 'processing' || status === 'processing') {
        return <div className="flex items-center justify-center">
            <h1>Class has ended, recording will be uploaded shortly, check back soon</h1>
        </div>
    }
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="text-white text-sm">
                                {isLive ? (!isBehindLive ?
                                    <> LIVE </>
                                    :
                                    <Button onClick={goToLive} variant="ghost" size="sm" className="text-red-500 hover:text-red-400 ml-2">
                                        Go Live
                                    </Button>
                                        ...
                                        <DropdownMenuPortal>
                                            <DropdownMenuSubContent sideOffset={-5} alignOffset={-5}>
                                                <DropdownMenuItem onSelect={() => handleQualityChange("auto")}>
                                                    <span>Auto</span>
                                                    {quality.label === "auto" && <Check className="ml-auto h-4 w-4" />}
                                                </DropdownMenuItem>
                                                {qualities.map((q) => (
                                                    <DropdownMenuItem key={q.label} onSelect={() => handleQualityChange(q.label)}>
                                                        <span>{q.label}</span>
                                                        {quality.label === q.label && <Check className="ml-auto h-4 w-4" />}
                                                    </DropdownMenuItem>
                                                ))}
                                            </DropdownMenuSubContent>
                                        </DropdownMenuPortal>
